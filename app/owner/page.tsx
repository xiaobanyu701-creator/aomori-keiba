...
export default function OwnerPage() {
  ...
  const [availableJockeys, setAvailableJockeys] = useState<string[]>([]);
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    const { data: existing } = await supabase.from("horse_masters").select("id").eq("name", ownerName).maybeSingle();
    if (existing) {
    ...
    if (h) {
      const updated = h.map((horse: any) => {
        ...
        return {
          ...
          ai_comment: horse.ai_comment || "状態良好です。",
        };
      });
      setHorses(updated);
      if (updated.length > 0) {
  ...
  const fetchJockeysFromRaces = async () => {
    const { data } = await supabase.from("race_horses").select("*");
    let jList: string[] = ["武豊", "ルメール", "川田将雅", "横山武史", "坂井瑠星", "松山弘平"];
    if (data && data.length > 0) {
      const dbJockeys = data.map((d: any) => d.jockey).filter((j): j is string => Boolean(j));
      jList = Array.from(new Set([...jList, ...dbJockeys]));
    }
  ...
  const handleBreed = async (type: "normal" | "random") => {
    ...
    const { data: setRes } = await supabase.from("system_settings").select("*").eq("key", "spawn_rates").maybeSingle();
